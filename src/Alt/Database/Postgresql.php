 ...
 */
class Alt_Database_Postgresql extends Alt_Database
{
	protected $_version;
	...
	public function connect()
	{
		...
		else
		{
			if ( ! pg_send_query($this->_connection, 'SET search_path = '.$this->_config['schema'].', pg_catalog'))
				throw new Alt_Exception(pg_last_error($this->_connection));

			if ( ! $result = pg_get_result($this->_connection))
				throw new Alt_Exception(pg_last_error($this->_connection));

			if (pg_result_status($result) !== PGSQL_COMMAND_OK)
				throw new Alt_Exception(pg_result_error($result));
		}
	}
	...
	public function query($type, $sql, $as_object = FALSE, array $params = NULL)
	{
		...
		try
		{
			if ($type === Alt_Database::INSERT AND $this->_config['primary_key'])
			{
			...
			switch (pg_result_status($result))
			{
				...
				case PGSQL_COPY_IN:
					...
					throw new Alt_Exception('PostgreSQL COPY operations not supported [ :query ]',
						array(':query' => $sql));
				default:
					$rows = 0;
			}
			...
			if ($type === Alt_Database::SELECT)
				return new Alt_Database_Postgresql_Result($result, $sql, $as_object, $params, $rows);

			if ($type === Alt_Database::INSERT)
			{
	...
	public function list_tables($like = NULL)
	{
		...
		return $this->query(Alt_Database::SELECT, $sql, FALSE)->as_array(NULL, 'table_name');
	}
	...
	public function list_columns($table, $like = NULL, $add_prefix = TRUE)
	{
		...
		foreach ($this->query(Alt_Database::SELECT, $sql, FALSE) as $column)
		{
